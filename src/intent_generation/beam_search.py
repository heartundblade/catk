...
class BeamSearch:
    ...
    def search(
        ...
        # print(valid_mask[:, 0:3])
        
        # Get first step's ground truth (only use for first step)
        first_step = self.model.num_historical_steps//5 + search_iter
        ...
        for agent_idx in range(n_agent):
            ...
            if tokenized_agent["type"][agent_idx] != 0:
                continue  # Only run beam search for vehicles
    ...
    def agent_token_emb_with_prev_feat(
        ...
        return feat_a, feat_a_t_dict

    def _search_single_agent_batch(
        self,
        tokenized_agent: Dict[str, torch.Tensor],
        ...
        """
        Perform beam search for a single agent with batch processing
        ...
        Args:
            ...
            first_step_gt_heading: Ground truth heading for first step
            first_step_gt_valid_mask: Ground truth validity for first step
        ...
        # Get current state
        pos_a = initial_state['pos_a'].clone()
        head_a = initial_state['head_a'].clone()
        t_now = pos_a.shape[1] - 1
        
        # Update all agents' states with ground truth in first step
        if full_tokenized_agent is not None:
            # For all agents, use ground truth at current step
            ...
            # Update agents' positions and headings to ground truth at the current step
        ...
        for step in range(1, self.max_length):
            ...
            # Update surrounding agents' states with ground truth where mask is True
            if current_gt_step < full_tokenized_agent['gt_pos'].shape[1]:
                ...
                temp_full_tokenized_agent['pos'][update_mask, t_now+1:t_now+2] = gt_pos_all[update_mask]
                temp_full_tokenized_agent['heading'][update_mask, t_now+1:t_now+2] = gt_head_all[update_mask]
            ...
            is_initial_step = step == 1
            if is_initial_step:
                feat_a = None
                feat_a_t_dict = None
            else:
                feat_a, feat_a_t_dict = self.agent_token_emb_with_prev_feat(
                    temp_full_tokenized_agent,
                    pred_dicts['feat_a'],
                    pred_dicts['feat_a_t_dict'],
                    beam
                )

            pred_dicts = self.model.encoder.agent_encoder.batch_inference_single_step(
                temp_full_tokenized_agent,
                map_feature,
                prev_feat_a=feat_a,
                prev_feat_a_t_dict=feat_a_t_dict,
        ...
        for step in range(1, self.max_length):
            ...
            for state in beam:
                ...
                if not is_initial_step:
                    ...
                    temp_full_tokenized_agent['pos'][:, t_now+1:t_now+2] = torch.where(
                        current_agent_mask.expand(-1, 1, 2),
                        pos_a[:, -1:].expand(n_agent, 1, 2),
                        temp_full_tokenized_agent['pos'][:, t_now+1:t_now+2]
                    )
                    temp_full_tokenized_agent['heading'][:, t_now+1:t_now+2] = torch.where(
                        current_agent_mask.squeeze(2),
    ...
    def _generate_child_state_with_gt(
        ...
        Returns:
            Child state
        """
        pos_a = parent_state['pos_a'].clone()
        head_a = parent_state['head_a'].clone()
        ...
        if gt_pos is not None and gt_head is not None:
            ...
            token_traj_global = transform_to_global(
                ...
                head_local=None,
                pos_now=tokenized_agent["pos"][:, t_now],
                head_now=tokenized_agent["heading"][:, t_now]
            ...
            pos_a_next_current = torch.masked_select(pos_a_next.unsqueeze(1), current_agent_mask)
            head_a_next_current = torch.masked_select(head_a_next.unsqueeze(1), current_agent_mask)
            
            # Update state
            pos_a = torch.cat([pos_a, pos_a_next_current.unsqueeze(0).unsqueeze(1)], dim=1)
            head_a = torch.cat([head_a, head_a_next_current.unsqueeze(0)], dim=1)
            pred_valid[:, n_step] = pred_valid[:, t_now]
    ...
    def closed_loop_search(
        ...
        gt_length = tokenized_agent["gt_pos"].shape[1]
        ...
        for search_iter in range(self.max_search_iterations):
            ...
            # Run one beam search
            search_result = self.search(tokenized_map, current_tokenized_agent, search_iter)
            ...
            new_pos = torch.cat([
                current_state['pos'],
                tokenized_agent['gt_pos'][:, current_step:current_step+1]
            ], dim=1)
            new_heading = torch.cat([
                current_state['heading'],
                tokenized_agent['gt_heading'][:, current_step:current_step+1]
            ], dim=1)
            new_idx = torch.cat([
                current_state['idx'],
                tokenized_agent['gt_idx'][:, current_step:current_step+1]
            ], dim=1)
            ...
            current_state = {
                'pos': new_pos,
                'heading': new_heading,
                'valid_mask': current_tokenized_agent['valid_mask'],
                'idx': new_idx
            }
